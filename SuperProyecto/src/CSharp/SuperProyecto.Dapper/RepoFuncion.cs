...
public class RepoFuncion : Repo, IRepoFuncion
{
    ...
    private static readonly string _queryUpdateFuncion
        = @"UPDATE Funcion SET idFuncion = @unIdFuncion, idEvento = @unIdEvento, descripcion = @unaDescripcion, fechaHora = @unaFechaHora";
    public void UpdateFuncion(Funcion funcion, int id)
    {
        _conexion.Execute(_queryUpdateFuncion, new { unIdFuncion = funcion.idFuncion, unIdEvento = funcion.idEvento, unaDescripcion = funcion.descripcion, unaFechaHora = funcion.fechaHora });
    }
}
