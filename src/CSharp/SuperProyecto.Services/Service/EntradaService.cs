...
public class EntradaService : IEntradaService
{
    ...
    public Result <string?> ValidarQr(int id)
    {
        var entrada = _repoEntrada.DetalleEntrada(id);
        if (entrada is null) return  Result <string?>.Ok ("Entrada no encontrada.");
        if (entrada.usada) return Result <string?>.Ok ("La entrada ya fue usada.");
